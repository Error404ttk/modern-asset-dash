...
-- Create policy for basic organization info - public access
...
-- Fix profiles table - prevent privilege escalation by removing update access for regular users
DROP POLICY IF EXISTS "Users can update their own profile" ON public.profiles;

-- Only super admins can update profiles (including role changes)
CREATE POLICY "Super admins can manage all profiles"
ON public.profiles
FOR ALL
USING (has_role(auth.uid(), 'super_admin'::app_role));

-- Users can only update non-sensitive fields through application layer with validation