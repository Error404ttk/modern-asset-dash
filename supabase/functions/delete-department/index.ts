...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Delete department request:', { id, reason });
    ...
    const { data: department, error: getDeptError } = await supabase
      .from('departments')
      .select('*')
      .eq('id', id)
      .single();

    if (getDeptError || !department) {
      return new Response(
        JSON.stringify({ success: false, error: 'ไม่พบข้อมูลหน่วยงาน' }),
        { 
          status: 404, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (deleteError) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'ไม่สามารถลบหน่วยงานได้' }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    await supabase
      ...
      .insert({
        ...
        old_value: JSON.stringify(department),
        new_value: null,
